...
class ocs_claim_report(osv.osv):
    """ CRM Claim Report"""

    _name = "ocs.claim.report"
    _auto = False
    ...
    _columns = {
        ...
        'neighborhood_id': fields.many2one('ocs.neighborhood', 'Neighborhood', readonly=True),
    ...
    def init(self, cr):
        ...
        cr.execute("""
            create or replace view ocs_claim_report as (
                select
                    min(c.id) as id,
                ...
                group by to_char(c.date, 'YYYY'), to_char(c.date, 'MM'),to_char(c.date, 'YYYY-MM-DD'),\
                        ...
                        c.categ_id,c.partner_id,c.neighborhood_id,c.create_date,
