...
3. Crear base de datos e instalar el módulo urban bridge usando el script utils/create_openerp_db.py (modificar acordemente):

   python utils/create_openerp_db.py -n urban -u openerp -p admin -l es_CO -g -i urban_bridge -s admin -c ~/git/openerp-idu-addons/openerp-server.conf

4. El módulo urban_bridge requiere las coordenadas geográficas cartesianas para Bogotá, por tanto se ejecuta en la base de datos la siguiente consulta:
